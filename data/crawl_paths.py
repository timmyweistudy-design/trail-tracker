...
REJECT_NAME = re.compile(
    ...
    # ⚠️ 路名也不可用「以路/街結尾」一刀切（見下方 ROAD_TAIL / TRAIL_MORPH）；這裡只擋
    #    明確的道路門牌形式「○○路一段」，不可寫成「段$」——會誤殺「龍過脈森林步道廍坑段」。
    r"\d+巷|\d+弄|\d+號$|[路街][一二三四五六七八九十]段$"
    # 產業道路／農路：是道路不是步道（放在路名判定之前，因為它們常含山區地名）
    r"|產業道路|農路"
    # 警語、狀態、指示語當名稱
    r"|禁止|止步|請勿|勿入|勿進|注意|中斷|不通|無法|封閉|遊客止|自行小心|拉繩|峭壁|艱難|路跡|未整|難行"
    # 路段／方向描述，不是步道識別名
    r"|^往|^通往|^接[^駁]|^下山|^上登|^陡上|^陡下|^陡峭|^溪谷路|^田間|捷徑|腰線|^捷|^叉$|^岔$"
    # 撤退／替代路段：是路線註記不是步道（原本靠市區路名規則誤打誤撞擋掉，該規則放寬後要明列）
    r"|下撤|撤退|替代路段|替代道路"
...
# 以「路／街／大道」結尾的名稱多半是市區道路，但台灣一堆真步道也這樣命名：
# 巴拉卡舊水管路、藤寮坑越嶺路、豹子厨越嶺保甲路、石笋公媽古道南路、內雙溪古道水路、
# 腰繞路、水圳路、台電保線路…。一刀切會把這些全砍掉（實測誤殺 16 條，最長 1.7km）。
# 改成：只有「不含任何步道語彙」的路名才擋。
ROAD_TAIL = re.compile(r"[路街]$|大道$")
TRAIL_MORPH = re.compile(r"古|越嶺|保甲|保線|水管|水圳|腰繞|圳|山|嶺|稜|徑|坑|寮|溪|林|步道|谷|崙|埔|崎")


def is_road_name(name):
    return bool(ROAD_TAIL.search(name)) and not TRAIL_MORPH.search(name)
...
def is_trail(name, surface, sac, tv, hws=()):
    if REJECT_NAME.search(name) or is_road_name(name):
        return False
...
KEEP = ["大坑1號步道", "樟湖5號步道", "鹿鳴吊橋步道", "桂花吊橋步道", "八通關越道：鹿鳴吊橋段",
        ...
        # 以「路」結尾但是真步道（曾被市區路名規則一刀切誤殺）
        "巴拉卡舊水管路", "藤寮坑越嶺路", "豹子厨越嶺保甲路", "石笋公媽古道南路", "內雙溪古道水路",
        "腰繞路", "石碇四尖腰繞路", "水圳路", "台電保線路", "東線越嶺路",
        # 以「段」結尾的真步道（不可用「段$」擋路名）
        "龍過脈森林步道廍坑段"]
DROP = ["成都路43巷", "中華路一段13巷", "中山北路一段140巷26弄", "禁止通行", "遊客止步", "步道中斷",
        ...
        # 真的是道路（不含任何步道語彙）
        "中華路一段", "榮華路", "陸軍路", "波濤路", "舊水泥路", "產業道路", "六坑產業道路", "香菇寮農路"]
...
def selftest():
    ...
    for n in KEEP:
        ...
        if m:
            ...
            bad.append(f"✗ 誤殺真步道「{n}」（被當成市區路名）")
    for n in DROP:
        if not (REJECT_NAME.search(n) or is_road_name(n)):
