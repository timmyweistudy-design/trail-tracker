...
// 清掉已不存在的步道：資料過濾規則變嚴後，舊 id 會留在這裡變成永遠查不到的死條目
// （實測擴充那次殘留 478 條）。mergeDetail 按 id 查所以不會顯示錯誤，但白白撐大延遲載入的檔案。
const live = new Set(trails.map(t => t.id));
let pruned = 0;
for (const id of Object.keys(detail)) {
  if (!live.has(id)) { delete detail[id]; pruned++; continue; }
  if (detail[id] && typeof detail[id] === "object") delete detail[id].waypoints;  // 先清舊的再重設（冪等）
}
...
console.log(`沿線地標注入 ${trailsWith} 條步道、共 ${total} 個地標；清掉已不存在的舊條目 ${pruned} 條 → web/js/trails-detail.js`);
