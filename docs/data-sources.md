...
安通越嶺古道東段、烏山嶺水利古道-西段）。沒座標的步道在 App 裡等於半殘：地圖不能定位、
「附近」篩不到、卡片算不出距離。

`build_data.borrow_coords()` 替它們向**同縣市**的同名社群步道借一個定位點：
- 先去掉「東段／西段／步道群」等後綴，再由長到短試前綴比對
  （官方與社群用字常差一兩字：官方「安通越嶺古道」vs OSM「安通越嶺道」，全名對不上、前綴才接得起來）
- **只借座標不借幾何**：「東段」對應到的是社群那條**完整**步道，把整條路線當成某一段的軌跡會誤導
